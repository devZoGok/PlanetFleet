...
namespace battleship
{
	...
	ActiveGameState::ActiveGameState(GuiAppState *gs, int plId) : AbstractAppState(
																  ...
																  guiState(gs), playerId(plId), depth(1)
	{
		initDragbox();
		...
		if (mainPlayer->getNumSelectedUnits() > 0)
		{
			...
			if (!forceCursorState)
			{
				...
				else if (roverSelected && (tradeCenter || refinery || lab) && (ownGameObj || (alliedGameObj && trade)))
				{
					...
					if (alliedGameObj)
						orderPossible = tradeRes && orderPossible;
			...
			else
			{
				...
				else if (cursorState == CursorState::LOAD || cursorState == CursorState::UNLOAD)
				{
					...
					bool loadResource = (cursorState == CursorState::LOAD ? canLoad : (rover->getLoad((int)ResourceType::REFINEDS) > 0 || rover->getLoad((int)ResourceType::WEALTH) > 0 || rover->getLoad((int)ResourceType::RESEARCH) > 0));

					bool transferResource = ((refinery && loadResource && (ownGameObj || (alliedGameObj && tradeRefs))) ||
	...
	bool ActiveGameState::isGameObjSelectable(GameObject *obj, bool useDragBox)
	{
		...
		for (int i = 0; i < selectionPoints.size(); i++)
		{
			...
			for (int j = 0; j < NUM_EDGES && (numAboveEdges == 0 || numBelowEdges == 0); j++)
			{
				...
				{
					if (vertA.x > selectionPoints[i].x)
						swap(vertA, vertB);
	...
	void ActiveGameState::renderUnits()
	{
		...
		for (Unit *u : units)
		{
			if (u->isVehicle() && ((Vehicle *)u)->getGarrisonable())
				continue;
	...
	{
		if (tradingScreen || !ConcreteGuiManager::getSingleton()->findClickedButtons().empty())
			return;
		...
		case Bind::ROTATE_OBJ_FRAME:
			...
			else if (!(isPressed || selectedUnits.empty()))
			{
				...
				else if (!isSelectionBox)
				{
					...
					{
						if (orderPossible)
							issueOrder(Order::TYPE::LOAD, vector<Order::Target>{Order::Target((Unit *)gameObjHoveredOn)}, shiftPressed);
					}
					else if (cursorState == CursorState::UNLOAD)
					{
						if (orderPossible)
							issueOrder(Order::TYPE::UNLOAD, vector<Order::Target>{Order::Target((Unit *)gameObjHoveredOn)}, shiftPressed);
					}
		...
		case Bind::EJECT_GARRISON:
			if (isPressed)
				issueOrder(Order::TYPE::EJECT, vector<Order::Target>{}, shiftPressed);
			break;
		case Bind::LAUNCH:
			if (isPressed)
			{
